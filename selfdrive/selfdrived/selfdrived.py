...
class SelfdriveD(CruiseHelper):
  def __init__(self, CP=None, CP_IQ=None):
    ...
    ignore = self.sensor_packets + self.gps_packets + ['alertDebug', 'iqModelDataV2', 'liveParameters', 'driverAssistance']
    if SIMULATION:
  ...
  def update_events(self, CS):
    ...
    if not self.CP.notCar:
      ...
      self.events_iq.add_from_msg(self._get_longitudinal_plan_ext().events)
    ...
    if CS.canValid:
      ...
      car_events_iq = self.car_events_iq.update(CS, self.events).to_msg()
      self.events_iq.add_from_msg(car_events_iq)
    ...
    if lane_turn_direction == TurnDirection.turnLeft:
      self.events_iq.add(custom.IQOnroadEvent.EventName.laneTurnLeft)
    elif lane_turn_direction == TurnDirection.turnRight:
      self.events_iq.add(custom.IQOnroadEvent.EventName.laneTurnRight)
